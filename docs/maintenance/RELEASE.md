...
新規パッケージの `package.json` 初期値は `0.0.0`（未リリース状態）とする。

| 初回リリースの方針 | 選択 | 結果 |
| --- | --- | --- |
| 開発版として公開する | `minor` | `0.0.0` → `0.1.0` |
| 正式版として公開する | `major` | `0.0.0` → `1.0.0` |

`0.x.x` 期間は開発版として位置づけ、破壊的変更を含む場合も `minor` でバンプしてよい。
`1.0.0` 以降は Semantic Versioning を厳密に適用する。
